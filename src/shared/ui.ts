...
export function buildStoryTeaser(story: PortalContent['storyTeaser']) {
  ...
  section.innerHTML = `<span class="kicker">Abendgeschichten</span><h2>${story.title}</h2><p>${story.text}</p>`;

  if (story.highlights?.length) {
    const tags = document.createElement('div');
    tags.className = 'story-teaser-tags';
    for (const highlight of story.highlights) {
      const chip = document.createElement('span');
      chip.className = 'story-teaser-chip';
  ...
  if (story.featuredStorySlugs?.length) {
    const links = document.createElement('div');
    links.className = 'story-teaser-links';
    for (const slug of story.featuredStorySlugs) {
      const link = document.createElement('a');
      link.className = 'link-muted';
      link.href = `/geschichten/${slug}/`;
      link.textContent = `→ ${slug.split('-').join(' ')}`;
      links.append(link);
    }
  ...
  action.href = story.route;
  action.textContent = 'Zum Geschichtenraum';
