...
export async function loadStoryHubMeta() {
  const response = await fetch('/content/stories/meta.json');
  return (await response.json()) as StoryHubMeta;
}

export async function loadStoryCategories() {
  const response = await fetch('/content/stories/categories.json');
  return (await response.json()) as StoryCategory[];
}

export async function loadStorySummaries() {
  const response = await fetch('/content/stories/stories.json');
  return (await response.json()) as StorySummary[];
}

export async function loadStoryWorld(storyId: string) {
  const response = await fetch(`/content/stories/story-worlds/${storyId}.json`);
  if (!response.ok) return undefined;
  return (await response.json()) as StoryWorld;
}
